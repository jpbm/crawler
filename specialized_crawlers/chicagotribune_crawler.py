...
def process_one(url):
    ...
    if response.status_code == 200:
        ...
        wait = randint(2*MAX_WORKERS)
        print('[process_one] Code: %i Too many requests. Sleeping %i seconds %s' % (response.status_code,int(wait),url))
        urls_to_do.update([url])
        sleep(wait)
        print('[process_one] Code: %i Too many requests. Resuming. %s' % (response.status_code,url))
    else:
        print('[process_one] Code: %i %s' % (response.status_code,url))
